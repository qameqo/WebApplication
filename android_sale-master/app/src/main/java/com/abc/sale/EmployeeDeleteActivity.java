...
public class EmployeeDeleteActivity extends AppCompatActivity {
    ...
    public void delete() {
        String url = getString(R.string.root_url) + getString(R.string.employee_delete_url) + empID;
        HttpUtils.delete(url, null, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    ...
                    if(status.equals("true")){
                        Toast.makeText(getApplicationContext(), "ลบข้อมูลเรียบร้อยแล้ว",
                                Toast.LENGTH_LONG).show();
                        back();
                    }else{
                        Toast.makeText(getApplicationContext(), "ไม่สามารถลบข้อมูลได้", Toast.LENGTH_LONG).show();
                        back();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
    ...
    public void update() {
        Intent intent = new Intent(getApplicationContext(), EmployeeUpdateActivity.class);
