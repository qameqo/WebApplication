<div class="container" style="text-align:center;">
  <br><br><br><br>

  <H1 class="font-weight-bolder">G Dragon Autotech</H1>

    <div class="row">

      <div class="col-sm-6">
        <div class="card text-center">
          <div class="card-body">
            <h4 class="card-title">จัดการข้อมูลบุคคล</h4>
            <a href="<?php echo site_url('Manager');?>" class="btn btn-primary btn-lg" style="font-size : 18px">แก้ไขข้อมูลส่วนตัว</a><br>
            <a href="<?php echo site_url('Manager_emp');?>" class="btn btn-primary btn-lg" style="font-size : 18px">จัดการข้อมูลพนักงาน</a>
          </div>
        </div>
      </div>

      <div class="col-sm-6">
        <div class="card text-center">
          <div class="card-body">
            <h4 class="card-title">จัดการข้อมูลเกี่ยวกับรถยนต์</h4>
            <a href="<?php echo site_url('Manager_car_regis');?>" class="btn btn-primary btn-lg" style="font-size : 18px">จัดการข้อมูลลงทะเบียนรถยนต์</a><br>
            <a href="<?php echo site_url('Manager_car_price');?>" class="btn btn-primary btn-lg" style="font-size : 18px">จัดการข้อมูลราคารถยนต์</a>
          </div>
        </div>
      </div>

    </div> <!--row-->


</div>